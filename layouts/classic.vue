                        ...
                        <v-list v-if="items2.childrens">
                          <v-list-tile v-for="(items3, idx3 ) in items2.childrens" :key="idx3">
                            <v-list-tile-title>
                              <span
                                router
                                @click="$router.push('/classic/'+items3.url), loadchart()"
                              >{{ items3.name }}</span>
                            </v-list-tile-title>
                          </v-list-tile>
                        </v-list>
        ...
        <v-container>
          <div class="float-right">
            <dataslastdraw
              :stocks="StockName"
              :StockData="getStockNewData($route.params.id)"
              :Reference="getReference($route.params.id)"
              :checkStock="checkStock"
              v-if="show1"
            />
          </div>
...
export default {
  ...
  data() {
    return {
      menuItems: [
        { title: "login", path: "/login", icon: "fingerprint" },
        { title: "regiser", path: "/register", icon: "group_add" },
        { title: "free trail", path: "/freetrail", icon: "playlist_play" }
      ],
