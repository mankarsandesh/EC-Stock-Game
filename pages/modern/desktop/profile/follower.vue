  ...
  <v-flex xs12 pt-5 pl-5>
    <v-flex xs10>
      <div class="title_container">
        <h3 class="text-black" v-if="followerListEmpty == true">{{$t('profile.noFollowers')}}</h3>
        <div class="follower_container" v-for="(data, index) in followerList" :key="index">
          ...
          <button class="btn_follow" v-on:click="
      ...
      try {
        ...
        if (res.code == 200) {
          ...
          if (this.countFollower == 0) {
            this.followerListEmpty = true;
          } else {
            this.followerListEmpty = false;
          }
        } else {
          this.followerListEmpty = true;
        }
