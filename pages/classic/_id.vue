<template>
  <div>
    ...
    <div
      v-if="$route.params.id ==this.$route.params.id.split('-')[0] + '-' + this.$route.params.id.split('-')[1] + '-' + this.$route.params.id.split('-')[2] +'-currentbet'"
    >
      <currentbet />
    </div>
    <div
      v-else-if="$route.params.id ==this.$route.params.id.split('-')[0] + '-' + this.$route.params.id.split('-')[1] + '-' + this.$route.params.id.split('-')[2] +'-history'"
    >
      <history />
    </div>
    <div
      v-else-if="$route.params.id ==this.$route.params.id.split('-')[0] + '-' + this.$route.params.id.split('-')[1] + '-' + this.$route.params.id.split('-')[2] +'-gameresult'"
    >
      <gameresult />
    </div>
    <div
      v-else-if="$route.params.id ==this.$route.params.id.split('-')[0] + '-' + this.$route.params.id.split('-')[1] + '-' + this.$route.params.id.split('-')[2] +'-announcement'"
    >
      <announcement />
    </div>
    <div
      v-else-if="$route.params.id ==this.$route.params.id.split('-')[0] + '-' + this.$route.params.id.split('-')[1] + '-' + this.$route.params.id.split('-')[2] +'-rule'"
    >
      <rule />
    </div>
    <div
      v-else-if="$route.params.id ==this.$route.params.id.split('-')[0] + '-' + this.$route.params.id.split('-')[1] + '-' + this.$route.params.id.split('-')[2] +'-setting'"
    >
      <setting />
    </div>
    ...
    <div v-else>
      ...
      >
        <v-tab
          class="text-sm-left text-whites"
          v-for="(item, idx1) in items"
          :key="idx1"
          :href="'#tab-' + item.name"
        >{{ $t('gamemsg.'+item.name )}}</v-tab>
      </v-tabs>
      ...
      <v-card>
        <v-layout row wrap>
          <v-flex xs12 md6>
            ...
            <v-btn
              @click="setPrice('confirm')"
              color="error"
              :disabled="this.betData.betdetails.length == '0'"
            >{{$t('msg.confirm')}}</v-btn>
          </v-flex>
          <v-flex xs12 md6>
          ...
          <v-card flat>
            <v-expansion-panel v-model="panel" expand>
              <v-expansion-panel-content
                class="bg-color"
                v-for="(items, idx4) in item.children"
                :key="idx4"
              >
                ...
                <v-card>
                  <table>
                    <tr>
                      <th
                        class="header-bet"
                        v-if="items.name == 'Specific-Number'"
                        :style="items.name == 'Specific-Number' && data=='bothdigit' ? 'width:40%':'width:20%'"
                        v-for="(data, idx6) in header"
                        :key="idx6"
                      >{{$t('gamemsg.'+data)}}</th>
                      <th
                        class="header-bet"
                        v-if="items.name !== 'Specific-Number'"
                        :style="items.name == 'Specific-Number' && data=='bothdigit' ? 'width:20%':'width:25%'"
                        v-for="(data, idx7) in header"
                        :key="idx7"
                      >{{$t('gamemsg.'+data)}}</th>
                    </tr>
                    ...
                    <tr v-for="(datas, idx5) in items.childrens" :key="idx5">
                      <td
                        class="top-bet"
                        :style="items.name == 'Specific-Number' ? 'width:20%':'width:25%'"
                        @click="betRow($event)"
                      >
                        <div
                          class="text-bet"
                        >{{datas.name >= 0 ? datas.name: $t('gamemsg.'+datas.name)}}</div>
                      ...
                      >
                        <div
                          class="text-bet"
                        >{{datas.name2 >= 0 ? datas.name2: $t('gamemsg.'+datas.name2)}}</div>
                        <div class="text-stock">{{items.payout}}</div>
                        <div class="bet-box">
                          <input
                            type="text"
                      ...
                      <td
                        ...
                        :style="items.name == 'Specific-Number' ? 'width:20%':'width:25%'"
                        @click="betRow($event)"
                        v-if="datas.childrenss"
                        v-for="(datass, idx8) in datas.childrenss"
                      >
                      ...
                      <td
                        ...
                        @click="betRow($event)"
                        v-if="datas.childrenss"
                        v-for="(datass, idx9) in datas.childrenss"
                        :key="idx9"
                      >
      ...
      >
        <v-tab
          class="text-sm-left text-whites"
          @click="loadchart()"
          v-for="(baccarat1, idx1) in baccarat"
          :key="idx1"
          :href="'#tab-' + baccarat1.name"
        >{{ baccarat1.name }}</v-tab>
      </v-tabs>

      <v-tabs-items v-model="currentItems">
        <v-tab-item
          v-for="(baccarat1, idx3) in baccarat"
          :key="idx3"
          :value="'tab-' + baccarat1.name"
        >
          <v-card flat>
            <v-tabs
              class="bg-colors"
              v-model="currentItemss"
              color="transparent"
              fixed-tabs
              slider-color="yellow"
              grow
            >
              <v-tab
                class="text-sm-left text-whites"
                @click="loadchart()"
                v-for="(baccarat2, idx11) in baccarat1.children"
                :key="idx11"
                :href="'#' + baccarat2.name"
              >{{ baccarat2.name }}</v-tab>
            </v-tabs>

            <baccarats
              :chtable="baccarat1.namech"
              :chlists="baccarat1.namech+'-'+currentItemss"
              :dataArray="getStockNewData($route.params.id)"
              :stocks="stockname"
            />
          </v-card>
        </v-tab-item>
      ...
      <v-dialog v-model="dialog" persistent max-width="1240px">
        <v-card>
            ...
            <table>
              ...
              <tr>
                <td colspan="2">
                  ...
                  <span
                    class="text-waring"
                  >{{this.betData.betdetails.length}}</span>
                </td>
          ...
          <v-card-actions>
            ...
            <v-btn
              color="success"
              @click="dialog = false, getConfirmBet()"
              :disabled="this.betData.betdetails.length == '0'"
            >{{$t('msg.ok')}}</v-btn>
                ...
                <td class="top-bet" @click="betRow($event)">
                  ...
                  <div class="bet-box">
                    <input
                      ...
                      @click="bet($event)"
                      :data-stock="stockname"
                      :name="10 > sntwoloopstart ? header[3]+'-0'+data : header[3]+'-'+data"
                    />
                  </div>
                </td>
              </tr>
