...
<v-layout wrap row>
    <v-flex xs2 lg2 v-if="!isFullscreen">
        <v-layout>
            <v-flex xs9>
                <v-btn block class="text-white firstDigit" color="#003e70" @click="changeChartType('firstDigit')">{{$t('gamemsg.firstdigits')}}</v-btn>
            </v-flex>
            <v-spacer></v-spacer>
        ...
        <v-layout>
            <v-flex xs9>
                <v-btn block class="text-white lastDigit" color="#003e70" @click="changeChartType('lastDigit')">{{$t('gamemsg.lastdigits')}}</v-btn>
            </v-flex>
            <v-spacer></v-spacer>
        ...
        <v-layout>
            <v-flex xs9>
                <v-btn block class="text-white bothDigit" color="#003e70" @click="changeChartType('bothDigit')">{{$t('gamemsg.bothdigits')}}</v-btn>
            </v-flex>
            <v-spacer></v-spacer>
        ...
        <v-layout>
            <v-flex xs9>
                <v-btn block class="text-white twoDigit" color="#003e70" @click="changeChartType('twoDigit')">{{$t('gamemsg.twodigits')}}</v-btn>
            </v-flex>
            <v-spacer></v-spacer>
    ...
    <v-flex class="xs10">
        <v-layout row wrap>
            <v-flex xs12 lg12 md12 ≈>
                <trendMap :dataArray="getStockCrawlerData($route.params.id)" :trendType="trendType" :key="getStockCrawlerData($route.params.id)[0].writetime + trendType" :isFullscreen="isFullscreen"></trendMap>
            </v-flex>
        </v-layout>
    </v-flex>
</v-layout>
