...
export default {
  ...
  methods: {
    ...
    async getChart() {
      ...
      this.betlose = [];
      this.betwon = [];
      let dataGet = await this.$axios.$post(
        "/api/me/betAnalysis?apikey=" + this.$store.state.provider.authToken
      );
      ...
      dataGet.data.forEach(element => {
        this.load = true;
        this.stockname.push(
          element.stockName == "btc1"
            ? this.$root.$t("stockname." + element.stockName) + " " + "1"
            : element.stockName == "btc5"
            ? this.$root.$t("stockname." + element.stockName) + " " + "5"
            : this.$root.$t("stockname." + element.stockName)
        );
